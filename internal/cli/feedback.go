...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".find-me-gluten-free-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FIND_ME_GLUTEN_FREE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FIND_ME_GLUTEN_FREE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/aclee555/find-me-gluten-free-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.find-me-gluten-free-pp-cli/feedback.jsonl.
When ` + "`FIND_ME_GLUTEN_FREE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FIND_ME_GLUTEN_FREE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "find-me-gluten-free-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  find-me-gluten-free-pp-cli feedback list
  find-me-gluten-free-pp-cli feedback list --limit 5
  find-me-gluten-free-pp-cli feedback list --json`,
