...
In local mode: searches locally synced data only.`,
  ...
  find-me-gluten-free-pp-cli search "error timeout"

  # Force local search only
  find-me-gluten-free-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  find-me-gluten-free-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  find-me-gluten-free-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'find-me-gluten-free-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/find-me-gluten-free-pp-cli/data.db)")
